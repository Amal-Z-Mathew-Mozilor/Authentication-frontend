...
| GET | `/pulse/websites/:id/cookie-policy` | — (`data.content` = `{ aboutCookies: {…}, useOfCookies: {…}, cookiePreferences: { heading, description }, effectiveDate, generatedAt? }` + `data.updatedAt` (ISO, last edit/generate — feeds the "Last updated" line); `generatedAt` present once generated — WebManager reads it to route preview vs wizard) |
...
| POST | `/pulse/websites/:id/cookie-policy/send-code` | `{ email }` (emails the HTML snippet to a teammate; `422` invalid email, `404` non-owned) |
