  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch(FORGOT_URL, {
        ...
        credentials: 'include',
        body: JSON.stringify({ email, resetBase: `${window.location.origin}/resetPassword` }),
      })
      ...
      if (res.ok) {
        setSentMsg(data.message || 'If the email exists, a reset link has been sent.')
        setStatus('sent')
        return
      }
      ...
      if (res.status === 422 && Array.isArray(data.errors)) {
        setErrors(data.errors.filter((er) => er.path === 'email').map((er) => er.msg))
        setStatus('idle')
        return
      }
